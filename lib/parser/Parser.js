...
const MinifyAstNodeTransformer = require('../transformers/MinifyAstNodeTransformer')
const DefaultAstNodeTransformer = require('../transformers/DefaultAstNodeTransformer')
 ...
 */
class Parser {
  constructor(grammar, minify, prefix, stopMap, parentParser) {
    ...
    this.nodeTransformer = minify
      ? new MinifyAstNodeTransformer()
      : new DefaultAstNodeTransformer()
  }
...
function getTokenName(node, transformer) {
  const type = node[transformer.transform('type')]
  ...
  if (type === TokenType.FunctionCall) {
    return (
      node[transformer.transform('value')] ||
      node[transformer.transform('name')]
    )
  }
...
Parser.findTokensByAstAndTypes = (ast, tokenTypes, includePosition) => {
  const nodeTransformer = ast.minified
    ? new MinifyAstNodeTransformer()
    : new DefaultAstNodeTransformer()
  ...
  while (nodeInProgress) {
    ...
    Object.values(nodeInProgress).forEach((childNode) => {
    ...
    if (
      tokenTypes.includes(nodeInProgress[nodeTransformer.transform('type')]) &&
      !nodeInProgress[nodeTransformer.transform('from')] &&
      !isNullOrUndefinedIdentifier(
        nodeInProgress[nodeTransformer.transform('value')]
      )
    ) {
      if (includePosition) {
        tokens.push({
...
Parser.findIdentifiersByAst = (ast, includePosition) => {
...
Parser.findFunctionNamesByAst = (ast, includePosition) => {
  return Parser.findTokensByAstAndTypes(
    ast,
