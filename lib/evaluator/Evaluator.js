...
class Evaluator {
  constructor(grammar, context, relativeContext, options, promise = Promise) {
    ...
    this.strictOptions = options && options.strictOptions
    this.emptyContextValue = options ? options.emptyContextValue : undefined
    this.emptySubjectValue = options ? options.emptySubjectValue : undefined

    this.scriptStartTimestamp = options && options.scriptStartTimestamp
    this._minified = options && options.minified
    this.timeout = options && options.timeout
    this.MAX_LENGTH_FOR_RELATIVE_FILTER = 200
    this.MAX_OBJECT_LITERAL_PROPERTIES = 1000

    this.nodeTransformer = this._minified
  ...
  eval(ast) {
    return this.Promise.resolve().then(() => {
      ...
      const type = ast[this.nodeTransformer.transform('type')]

      return handlers[type].call(this, ast)
    })
  }
  ...
  evalMap(map) {
    ...
    if (keys.length > this.MAX_OBJECT_LITERAL_PROPERTIES) {
      throw new ExecutionError(
        `Object literal can have up to ${this.MAX_OBJECT_LITERAL_PROPERTIES}`
      )
    }
  ...
  _filterRelative(subject, expr) {
    ...
    if (subject.length > this.MAX_LENGTH_FOR_RELATIVE_FILTER) {
      throw new ExecutionError(
        `Subject cannot have more than ${this.MAX_LENGTH_FOR_RELATIVE_FILTER}`
      )
    }
    ...
    subject.forEach((elem) => {
      const evalInst = new Evaluator(
        ...
        elem,
        {
          ...
          emptyContextValue: this.emptyContextValue,
          timeout: this.timeout,
          scriptStartTimestamp: this.scriptStartTimestamp,
  ...
  _filterStatic(subject, expr) {
    return this.eval(expr).then((res) => {
      ...
      if (this.shouldThrowErrorIfReadPropertyOfNullOrUndefined()) {
        throw new ReadFromEmptyObjectError(res, subject)
      }

      return this.emptySubjectValue
    })
  }

  _verifyTimeout() {
    if (!this.timeout || !this.scriptStartTimestamp) {
      return
    }

    const currentTimestamp = Date.now()

    if (currentTimestamp - this.scriptStartTimestamp > this.timeout) {
      throw TimeoutError(
        `Execution timeout after ${
          currentTimestamp - this.scriptStartTimestamp
        }`
      )
    }
  }

  shouldThrowErrorIfVariableIsNotFound() {
    return (
      this.strictMode &&
      (!this.strictOptions ||
        !this.strictOptions.disableThrowVariableNotFoundError)
    )
  }

  shouldThrowErrorIfReadPropertyOfNullOrUndefined() {
    return (
      this.strictMode &&
      (!this.strictOptions ||
        !this.strictOptions.disableThrowReadPropertyOfNullOrUndefinedError)
    )
  }
}
