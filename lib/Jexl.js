...
class Jexl {
  ...
  createExpression(expression, ast) {
    return new Expression(this._grammar, expression, ast)
  }
   ...
   * @returns {Promise<*>} resolves with the result of the evaluation.
   */
  evalPreCompiled(expressionStr, ast, context) {
    const expression = this.createExpression(expressionStr, ast)
   ...
   * @throws {*} on error
   */
  evalSyncPreCompiled(expressionStr, ast, context) {
    const expression = this.createExpression(expressionStr, ast)
