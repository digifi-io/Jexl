...
class Jexl {
  ...
  createExpression(expression, options) {
    return new Expression(this._grammar, expression, undefined, options)
  }

  /**
   * Constructs an Expression object from a Jexl expression string.
   * @param {object} ast Compiled Abstract Syntax Tree object
   * @param {object=} options Evaluator options
   * @returns {Expression} The Expression object representing the given string
   */
  createExpressionFromAst(ast, options) {
    return new Expression(this._grammar, undefined, ast, options)
  }
  ...
  /**
   * Retrieves available function names.
   * @returns {string[]} Function names
   */
  getAvailableFunctionsNames() {
    return Object.keys(this._grammar.functions)
  }
  ...
  /**
   * Asynchronously evaluates a Jexl string within AST object and an optional context.
   ...
   * @returns {Promise<*>} resolves with the result of the evaluation.
   */
  evalPreCompiled(ast, context = {}, options = {}) {
    const expression = this.createExpressionFromAst(ast, options)

    return expression.eval(context)
  }

  /**
   * Synchronously evaluates a Jexl string within AST object and an optional context.
   * @param {Object} ast Abstract Syntax Tree object
   * @param {Object} [context] A mapping of variables to values, which will be
   *      made accessible to the Jexl expression when evaluating it
   * @param {Object} [options] Evaluator Options
   ...
   * @throws {*} on error
   */
  evalSyncPreCompiled(ast, context = {}, options = {}) {
    const expression = this.createExpressionFromAst(ast, options)

    return expression.evalSync(context)
  }

  findIdentifiersByAst(ast) {
    return Parser.findIdentifiersByAst(ast)
  }

  findFunctionNamesByAst(ast) {
    return Parser.findFunctionNamesByAst(ast)
  }

  findIdentifiersWithPositionByAst(ast) {
    return Parser.findIdentifiersByAst(ast, true)
  }

  findFunctionNamesWithPositionByAst(ast) {
    return Parser.findFunctionNamesByAst(ast, true)
  }

  findTokensByAstAndTypes(ast, tokenTypes) {
    return Parser.findTokensByAstAndTypes(
      ast,
      tokenTypes.map((type) => {
        return type in TokenType ? TokenType[type] : type
      }),
      true
    )
  }
