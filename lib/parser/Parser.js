...
Parser.findTokensByAstAndTypes = (ast, tokenTypes, includePosition) => {
  ...
  while (nodeInProgress) {
    ...
    if (
      tokenTypes.includes(nodeInProgress.type) &&
      !nodeInProgress.from &&
      !isNullOrUndefinedIdentifier(nodeInProgress.value)
    ) {
      if (includePosition) {
        tokens.push({
          name: nodeInProgress.value || nodeInProgress.name,
          type: nodeInProgress.type,
          position: { start: nodeInProgress.start, end: nodeInProgress.end }
        })
      } else {
        tokens.push(nodeInProgress.value)
      }
    }
...
Parser.findIdentifiersByAst = (ast, includePosition) => {
  return Parser.findTokensByAstAndTypes(ast, ['Identifier'], includePosition)
}
